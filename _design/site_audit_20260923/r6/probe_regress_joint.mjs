...
// 사용: node probe_regress_joint.mjs [dsf] → r6/regress_joint.json (dsf 1), dsf 2 이상이면 r6/regress_joint_dsf<N>.json, shots/joint_dsf<N>/. 행 좌표는 CSS px(= 캡처 행 / dsf).
...
const PRE = "94f577b", POST = "3d2ada3";
...
for (const variant of ["pre", "post"]) {
  for (const [w, h] of VPS) {
    ...
    for (const target of ["index", "about"]) {
      ...
      if (variant === "pre") await ctx.route(u => u.host === "localhost:8092" && FILES.includes(u.pathname.slice(1)), r => r.fulfill({ status: 200, headers: { "content-type": TYPES[r.request().url().split(".").pop()], "cache-control": "no-store" }, body: preBody[new URL(r.request().url()).pathname.slice(1)] }));
