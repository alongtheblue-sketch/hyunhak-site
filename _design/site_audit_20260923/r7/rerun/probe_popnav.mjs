// critic 7회차 프로브 popnav (2026-09-23): ab9160d (3d2ada3 위 재작업: .pbar margin-top -1px, fitBar --pbar-h, layout() 감아 돌기 transition none + data-far, onArrow .pdim 초점 키 스크롤) 검증. 라이브 = 96be167(팝업 코드 = 94f577b).
//   r6/probe_popnav.mjs 복사본. 바꾼 것 = 산출 경로(r7/) 와 「하단바 위치」 기대: 6회차 「하단바 top = 활성 .pop bottom(±0.5)」 →
//   이번 「하단바 top = 활성 .pop bottom - 1px(±0.5, margin-top -1 겹침)」. 접합 crop 의 선 기대 행도 하단바 top 에서 계산한다.
//   더한 것 = nav Space 경로 keydown defaultPrevented 기록, restpos 1행 높이 = 하단바 top - margin-top, short 7종(568x320, 320x256 추가) + --pbar-h 대조 + 끝까지 스크롤,
//   wrap(신규, N=3/N=4 감아 돌기 rAF 표본), keys(신규, .pdim 초점 키 스크롤과 단추 Space), visual 접합 dsf3 를 1440/390 × 카드 둘로.
...
//   8 short   390x640, 360x640, 844x390(가로 폰), 568x320, 320x256, 1440x600, 1280x720 × N=2/N=3 × 카드별: 하단바 뷰포트 안, 활성 top ≥ 0, --pbar-h = 하단바 실측 높이,
//             카드 max-height 안 끝까지 스크롤(마지막 내용 요소가 하단바 위에 보임), 측면 카드와 하단바 겹침
//   9 wrap    (7회차 신규) N=3(공지 route) / N=4(의뢰 카드 복제 2장 addInitScript) × 1920x1080, 1440x900 × 기본/reduce × → N회, ← N회:
//             키 직후 동기 표본 + rAF 프레임 전수(약 450ms) 로 측면 슬롯이 활성 카드 폭 안으로 들어오는지, 감아 도는 슬롯 inline transition none → 제거, |d|≥2 data-far + opacity 0 + pointer-events none
//   10 keys   (7회차 신규) 1440x420, 390x640 × N=2 첫 초점 .pdim: ArrowDown/PageDown/Space/ArrowUp/Shift+Space/PageUp 의 scrollTop 변화(clamp 기대값 대조)와 초점 유지,
//             ArrowRight/ArrowLeft 넘김, Tab 으로 카드 안 조작 요소와 첫 <button> 에 초점을 옮긴 뒤 Space(가로채기 없음, 단추 눌림)
// 사용: node probe_popnav.mjs [nav|seq|sweep|restpos|pause|n1|visual|close|focus|short|wrap|keys|all]  → r7/popnav.json (구획별 병합), r7/shots/popnav_*.png
...
// 서빙 파일 = HEAD 인지(하네스가 ab9160d 를 내는지) 대조
{
...
const MUT = () => {
  ...
  // 7회차: keydown 이 문서 처리기(onArrow) 뒤에 창까지 올라왔을 때 defaultPrevented 와 대상(= 누를 때 초점 요소)
  window.__kd = [];
  window.addEventListener("keydown", (e) => { const t = e.target; window.__kd.push({ key: e.key, shift: e.shiftKey, dp: e.defaultPrevented, tgt: (t.tagName || "").toLowerCase() + (typeof t.className === "string" && t.className ? "." + t.className.trim().split(/\s+/)[0] : "") + (t.getAttribute && t.getAttribute("aria-label") ? "[" + t.getAttribute("aria-label") + "]" : ""), tgtIsRoot: t === document.querySelector(".pdim"), t: +performance.now().toFixed(1) }); });
  window.__hov = [];
...
// 기하 판정(정지 뒤): 7회차 = 하단바 top = 활성 카드 bottom - 1px(margin-top -1 겹침) ±0.5px, 좌우 끝 ±0.5px. barGap = bar.top - pop.bottom 이라 기대 -1
const GAP_EXP = -1;
const gapOk = (g) => typeof g === "number" && Math.abs(g - GAP_EXP) <= 0.5;
const barAligned = (s) => !!s && s.open && s.barGap !== null && gapOk(s.barGap) && Math.abs(s.barDL) <= 0.5 && Math.abs(s.barDR) <= 0.5;
...
const GEO = () => {
  ...
  return {
    ...
    barBorderTop: bc && bc.borderTopWidth + " " + bc.borderTopStyle + " " + bc.borderTopColor, barBorderSide: bc && bc.borderLeftWidth + " " + bc.borderLeftColor, barMarginTop: bc && bc.marginTop,
...
// 6회차: 활성 .pop 아래 선 = 폭 0 대신 투명(3d2ada3). 둘 중 하나면 겹선 없음. 7회차: gap 기대 -1(±0.5)
const joinOk = (g) => !!g && (g.popBorderBottom.startsWith("0px") || /rgba\(0, 0, 0, 0\)|transparent/.test(g.popBorderBottom)) && !!g.barBorderTop && g.barBorderTop.startsWith("1px solid") && gapOk(g.gap) && Math.abs(g.dL) <= 0.5 && Math.abs(g.dR) <= 0.5;
...
async function joinCrop(page, file) {
  ...
  const clip = { x: Math.round(g.pop.l - 8), y: Math.round(g.pop.b - 30), width: 140, height: 60 };   // 7회차: 정수 CSS 좌표로 자른다(선 기대 행 계산을 단순히)
  ...
  const scan = Object.fromEntries(Object.entries(px).map(([x, col]) => {
    ...
    return [x, { line, aboveMaxDiff: Math.max(0, ...above.map(diff)), aboveDiffRowsPx: above.filter((c) => diff(c) > 2).length, belowFirst: below[0] ? below[0].join(",") : null, belowLast: below.at(-1) ? below.at(-1).join(",") : null, belowShadowRowsPx: shadowRows, belowMaxDiff: Math.max(0, ...below.map(diff)) }];
  }));
  // 7회차: 선 기대 행 = 하단바 top(= pop.b - 1) 의 crop 안 장치 행. 6회차는 pop.b 고정(30*3)
  const junctionDevY = Math.round((g.bar.t - clip.y) * 3);
  // 딤 비침 = 선 밖(위 카드 바탕, 아래 하단바 바탕)에서 바탕과 2 넘게 다른 행 수. 선 런 자체는 뺀다
  for (const s of Object.values(scan)) s.dimRowsPx = s.line.length === 1 ? s.aboveDiffRowsPx + s.belowShadowRowsPx : null;
  const ok = Object.values(scan).every((s) => s.line.length === 1 && s.line[0].lenPx === 3 && new Set(s.line[0].colors).size === 1 && Math.abs(s.line[0].startPx - junctionDevY) <= 1 && s.aboveMaxDiff <= 2 && s.belowMaxDiff <= 2);
  return { file: p, clip, geo: g, junctionDevY, scan, singleLine3px: ok, noShadowOnBar: Object.values(scan).every((s) => s.belowShadowRowsPx === 0) };
}
...
async function runPath(w, reduce, name, N) {
  ...
  try {
    ...
    await page.evaluate(() => { window.__mut.splice(0); window.__kd.splice(0); });
    if (A.side) {
    ...
    const kd = await page.evaluate(() => window.__kd.splice(0));
    // 7회차: 초점이 단추(→/←)일 때 Space 는 가로채지 않고(defaultPrevented false) 그 단추를 누른다(turned). 가로채기는 초점이 .pdim 일 때만(keys 구획)
    if (name.startsWith("space_")) { const sp = kd.filter((k) => k.key === " "); v.spaceOnButtonNotIntercepted = sp.length === 1 && sp[0].dp === false && !sp[0].tgtIsRoot && /^button/.test(sp[0].tgt); }
    const pass = Object.values(v).every((x) => x === null || x === true);
    return { w, reduce, N, setup, load: { cur: load.cur, focus: load.focus.el, pause: load.pause }, before: { cur: before.cur, focus: before.focus.el, pause: before.pause }, target, info, kd,
...
async function navPart() {
  ...
  out.summary = { total: all.length, pass: all.filter(([, r]) => r.pass).length, fail: all.filter(([, r]) => !r.pass).map(([k, r]) => ({ k, bad: Object.entries(r.v).filter(([, x]) => x === false).map(([n]) => n) })),
    ...
    barGapLateRange: [Math.min(...all.map(([, r]) => r.late.barGap ?? 99)), Math.max(...all.map(([, r]) => r.late.barGap ?? 99))], maxBarGapDevFromExpLate: +Math.max(...all.map(([, r]) => Math.abs((r.late.barGap ?? 99) - GAP_EXP))).toFixed(2),
    spaceOnButton: all.filter(([k]) => k.startsWith("space_")).map(([k, r]) => [k, r.v.spaceOnButtonNotIntercepted]).filter(([, x]) => x !== true).length + " fail / " + all.filter(([k]) => k.startsWith("space_")).length,
    maxBarMove: Math.max(...all.map(([, r]) => Math.max(r.bar.dAfter ?? 99, r.bar.dLate ?? 99))) };
...
const RESTROW = () => {
  ...
  const mt = parseFloat(getComputedStyle(bar).marginTop) || 0;   // 7회차: 하단바 margin-top -1 → 2행 시작 = 하단바 top - margin-top
  const row1H = br.top - mt - sr.top;   // 무대 1행 = 슬롯 칸(2행 = 하단바). stage 는 padding 0
  return { slot: +on.dataset.i, title: on.querySelector("h2").textContent.trim().slice(0, 18), popH: +pr.height.toFixed(1), popTop: +pr.top.toFixed(1), popBottom: +pr.bottom.toFixed(1),
    barTop: +br.top.toFixed(1), barBottom: +br.bottom.toFixed(1), barMarginTop: mt, barGap: +(br.top - pr.bottom).toFixed(2), row1H: +row1H.toFixed(1), emptyAbove: +(row1H - pr.height).toFixed(1),
...
async function restposPart() {
  ...
  for (const reduce of [true, false]) {
    for (const [w, N] of [[1440, 2], [1440, 3], [390, 2], [390, 3]]) {
      ...
      out[key] = { rows, nextXSpread: +(Math.max(...xs) - Math.min(...xs)).toFixed(1), nextYSpread: +(Math.max(...ys) - Math.min(...ys)).toFixed(1), firstPointCoveredBy: covers,
        barGaps: rows.map((r) => r.barGap), barAlignedAll: rows.every((r) => gapOk(r.barGap)), emptyAbove: rows.map((r) => r.emptyAbove), popTops: rows.map((r) => r.popTop) };
  ...
  out.summary = { maxNextXSpread: Math.max(...all.map((x) => x.nextXSpread)), maxNextYSpread: Math.max(...all.map((x) => x.nextYSpread)), barAlignedAll: all.every((x) => x.barAlignedAll),
    barGapRange: [Math.min(...all.flatMap((x) => x.barGaps)), Math.max(...all.flatMap((x) => x.barGaps))], maxBarGapDevFromExp: +Math.max(...all.flatMap((x) => x.barGaps.map((g) => Math.abs(g - GAP_EXP)))).toFixed(2),
    emptyAbove: Object.fromEntries(Object.entries(out).filter(([, x]) => x && x.rows).map(([k, x]) => [k, x.emptyAbove])) };
...
async function visualPart() {
  ...
  { // 7회차: 390 dsf3 접합(카드 둘)
    const { ctx, page } = await open(390, { dsf: 3 });
    out.join390Rest = await joinCrop(page, "popnav_join_x3_390.png");
    await page.tap(sel("next")); await page.waitForTimeout(800);
    out.join390AfterNext = await joinCrop(page, "popnav_join_x3_390_after_next.png");
    log("visual join 390", out.join390Rest.singleLine3px, out.join390AfterNext.singleLine3px, JSON.stringify([out.join390Rest.scan, out.join390AfterNext.scan].map((s) => Object.values(s).map((c) => [c.line.map((l) => [l.startPx, l.lenPx, l.colors[0]]), c.aboveMaxDiff, c.belowMaxDiff]))), "exp", out.join390Rest.junctionDevY, out.join390AfterNext.junctionDevY);
    await ctx.close();
  }
  out.joinSummary = Object.fromEntries(["joinRest", "joinAfterNext", "join390Rest", "join390AfterNext"].map((k) => [k, { single3px: out[k].singleLine3px, noShadowOnBar: out[k].noShadowOnBar, gap: out[k].geo.gap, expDevY: out[k].junctionDevY,
    cols: Object.fromEntries(Object.entries(out[k].scan).map(([x, c]) => [x, { runs: c.line.map((l) => ({ start: l.startPx, len: l.lenPx, color: [...new Set(l.colors)] })), aboveMaxDiff: c.aboveMaxDiff, belowMaxDiff: c.belowMaxDiff, dimRowsPx: c.dimRowsPx }])) }]));
...
// ───────── 8 짧은 화면 (6회차 신규) ─────────
const SHORT = () => {
  const R = (b) => ({ l: +b.left.toFixed(1), t: +b.top.toFixed(1), r: +b.right.toFixed(1), b: +b.bottom.toFixed(1), h: +b.height.toFixed(2) });
  ...
  // 7회차: --pbar-h(.pdim inline) 와 하단바 실측 높이, 기대 max-height = min(72vh, vh - 2*16 - --pbar-h)
  const pbarVar = root.style.getPropertyValue("--pbar-h"), pbarPx = parseFloat(pbarVar);
  const maxHExp = Math.min(0.72 * innerHeight, innerHeight - 32 - (Number.isFinite(pbarPx) ? pbarPx : 0));
  // 끝까지 스크롤: scrollTop = 최대 → 마지막 내용 요소(.pfr 의 마지막 자손 요소)가 카드 안 보이는 영역에 끝까지 들고, 하단바 top 위에 있는가
  const leaf = (() => { let e = pop.querySelector(".pfr"); while (e && e.lastElementChild) e = e.lastElementChild; return e; })();
  pop.scrollTop = sh; const stMax = pop.scrollTop;
  const lr = leaf ? leaf.getBoundingClientRect() : null, prNow = pop.getBoundingClientRect();
  const bl = parseFloat(cs.borderBottomWidth) || 0;
  const end = { stMax, expMax: sh - ch, reachedEnd: Math.abs(stMax - (sh - ch)) <= 1, leaf: leaf ? leaf.tagName.toLowerCase() + (leaf.className ? "." + String(leaf.className).split(/\s+/)[0] : "") + "«" + (leaf.textContent || "").trim().slice(0, 12) + "»" : null,
    leafBottom: lr ? +lr.bottom.toFixed(1) : null, popInnerBottom: +(prNow.bottom - bl).toFixed(1), leafAboveBar: lr ? lr.bottom <= br.top + 0.5 : null, leafInsidePop: lr ? lr.bottom <= prNow.bottom - bl + 0.5 && lr.top >= prNow.top - 0.5 : null };
  pop.scrollTop = s0;
  return { vw: innerWidth, vh: innerHeight, slot: +on.dataset.i, title: on.querySelector("h2").textContent.trim().slice(0, 16), bar: R(br), barInView: br.bottom <= innerHeight + 0.01 && br.top >= -0.01, barOverflowPx: +(br.bottom - innerHeight).toFixed(1),
    barOffsetH: bar.offsetHeight, pbarVar, pbarMatchesOffsetH: pbarPx === bar.offsetHeight, pbarMinusRectH: +(pbarPx - br.height).toFixed(2),
    pop: R(pr), popTopInView: pr.top >= -0.01, stage: R(stage.getBoundingClientRect()), maxHeight: cs.maxHeight, maxHExpected: +maxHExp.toFixed(2), maxHMatches: Math.abs(maxH - maxHExp) <= 0.5, popWithinMax: pr.height <= (Number.isFinite(maxH) ? maxH : 0.72 * innerHeight) + 0.5,
    overflowY: cs.overflowY, scrollH: sh, clientH: ch, overflows: sh > ch + 1, progScroll: s1 - s0, end, barGap: +(br.top - pr.bottom).toFixed(2), sides };
};
async function shortPart() {
  const out = {};
  const VPS = [[390, 640, true], [360, 640, true], [844, 390, true], [568, 320, true], [320, 256, true], [1440, 600, false], [1280, 720, false]];   // 7회차: 568x320, 320x256 추가
    ...
    for (const N of [2, 3]) {
      ...
      for (let k = 0; k < N; k++) {
        ...
        if (r.overflows) {
          ...
          // 7회차: 실조작으로 끝까지(큰 휠 / 긴 터치 제스처) → scrollTop 이 최대에 닿는가
          if (touch) {
            const cdp = await ctx.newCDPSession(page);
            await cdp.send("Input.synthesizeScrollGesture", { x: c.x, y: c.y, yDistance: -1500, speed: 3000, gestureSourceType: "touch", repeatCount: 2 }).catch((e) => { r.gestureErr2 = e.message; });
            await cdp.detach().catch(() => {});
          } else { await page.mouse.move(c.x, c.y); for (let k = 0; k < 6; k++) { await page.mouse.wheel(0, 400); await page.waitForTimeout(60); } }
          await page.waitForTimeout(500);
          r.realEnd = await page.evaluate(() => { const p = document.querySelector(".pdim .pslot[data-on] > .pop"); return { st: p.scrollTop, max: p.scrollHeight - p.clientHeight }; });
          r.realEnd.reached = Math.abs(r.realEnd.st - r.realEnd.max) <= 1;
          r.pageScrolledEnd = (await page.evaluate(() => scrollY)) - winY0;
          r.stillOpenEnd = !!(await page.$(".pdim"));
      ...
      for (const r of rows) r.v = { barInView: r.barInView, popTopInView: r.popTopInView, pbarEqBarH: r.pbarMatchesOffsetH && Math.abs(r.pbarMinusRectH) <= 0.5, maxHMatches: r.maxHMatches, popWithinMax: r.popWithinMax, barGap: gapOk(r.barGap),
        sidesNoOverlap: r.sides.every((s) => s.overlapArea === 0), endVisible: r.end.reachedEnd && r.end.leafInsidePop && r.end.leafAboveBar,
        scroll: !r.overflows || (r.progScroll > 0 && r.realScroll > 0 && r.stillOpen && r.pageScrolled === 0 && r.curAfterScroll === r.slot + 1 && r.realEnd.reached && r.stillOpenEnd && r.pageScrolledEnd === 0) };
      const pass = rows.every((r) => Object.values(r.v).every(Boolean));
      out[key] = { rows, pass, errors };
      log(`short ${key}`, pass ? "PASS" : "FAIL", JSON.stringify(rows.map((r) => ({ slot: r.slot, bar: [r.bar.t, r.bar.b, r.bar.h, r.vh], pbar: [r.pbarVar, r.barOffsetH], barOver: r.barOverflowPx, popT: r.pop.t, popH: r.pop.h, maxH: [r.maxHeight, r.maxHExpected], of: r.overflows, prog: r.progScroll, real: r.realScroll, end: r.realEnd && [r.realEnd.st, r.realEnd.max], leaf: [r.end.leafBottom, r.end.popInnerBottom, r.bar.t], pageSc: r.pageScrolled, cur: r.curAfterScroll, gap: r.barGap, sides: r.sides.map((s) => [s.i, s.overlapArea, s.slotOpacity, s.gapAboveBar]), bad: Object.entries(r.v).filter(([, x]) => !x).map(([k]) => k) }))));
      await ctx.close();
    }
  }
  const all = Object.values(out).filter((x) => x && x.rows);
  out.summary = { pass: all.every((x) => x.pass), fail: Object.entries(out).filter(([, x]) => x && x.rows && !x.pass).map(([k]) => k),
    ...
    scrolledOk: all.flatMap((x) => x.rows.filter((r) => r.overflows && r.realScroll > 0)).length, realEndOk: all.flatMap((x) => x.rows.filter((r) => r.overflows && r.realEnd && r.realEnd.reached)).length,
    cards: all.flatMap((x) => x.rows).length, pbarEqOk: all.flatMap((x) => x.rows.filter((r) => r.v.pbarEqBarH)).length, endVisibleOk: all.flatMap((x) => x.rows.filter((r) => r.v.endVisible)).length,
    maxPbarMinusRectH: Math.max(...all.flatMap((x) => x.rows.map((r) => Math.abs(r.pbarMinusRectH)))), barGapRange: [Math.min(...all.flatMap((x) => x.rows.map((r) => r.barGap))), Math.max(...all.flatMap((x) => x.rows.map((r) => r.barGap)))] };
  log("short summary", JSON.stringify(out.summary));
  result.short = out; save();
}

// ───────── 9 감아 돌기 (7회차 신규) ─────────
//   N=3 = 공지 1건 route. N=4 = 의뢰 카드 복제 2장(r7/extra_check.mjs clone 과 같은 방법) + 공지 0건 route.
//   키마다: 창 keydown(문서 onArrow 뒤) 에서 동기 표본(inline transition, data-d, data-far) → rAF 프레임 전수 약 450ms(슬롯 rect, computed opacity, pointer-events, inline transition) → 700ms 정지 상태.
const CLONE4 = () => document.addEventListener("DOMContentLoaded", () => {
...
const WRAP_ARM = () => {
  ...
  const prevOn = slots.findIndex((s) => s.hasAttribute("data-on"));
  const rr = slots[prevOn].getBoundingClientRect();
  const snap = (withRect) => slots.map((s, i) => {
    const o = { i, d: s.dataset.d === undefined ? null : Number(s.dataset.d), on: s.hasAttribute("data-on"), far: s.hasAttribute("data-far"), tr: s.style.transition || "" };
    if (withRect) { const r = s.getBoundingClientRect(), cs = getComputedStyle(s); o.l = +r.left.toFixed(1); o.r = +r.right.toFixed(1); o.op = +parseFloat(cs.opacity).toFixed(3); o.pe = cs.pointerEvents; }
    return o;
  });
  window.__wrap = { prevOn, prevD: snap(false).map((x) => x.d), rest: { l: +rr.left.toFixed(1), r: +rr.right.toFixed(1) }, sync: null, frames: [], done: false };
  const onKey = () => {
    window.removeEventListener("keydown", onKey);
    const t0 = performance.now();
    window.__wrap.sync = snap(false);
    const f = () => { const t = performance.now() - t0; window.__wrap.frames.push({ t: +t.toFixed(1), s: snap(true) }); if (t < 450) requestAnimationFrame(f); else window.__wrap.done = true; };
    requestAnimationFrame(f);
  };
...
const WRAP_REST = () => {
  ...
  return slots.map((s, i) => {
    ...
    const vis = { l: Math.max(0, r.left), r: Math.min(innerWidth, r.right) };
    let hit = null;
    if (s.hasAttribute("data-far") && vis.r - vis.l > 4) { const e = document.elementFromPoint((vis.l + vis.r) / 2, (r.top + r.bottom) / 2); hit = e ? { inSlot: s.contains(e), el: e.tagName.toLowerCase() + "." + (typeof e.className === "string" ? e.className.split(/\s+/)[0] : "") } : null; }
    return { i, d: Number(s.dataset.d), on: s.hasAttribute("data-on"), side: s.getAttribute("data-side"), far: s.hasAttribute("data-far"), op: +parseFloat(cs.opacity).toFixed(3), pe: cs.pointerEvents, tr: s.style.transition || "", l: +r.left.toFixed(1), r: +r.right.toFixed(1), visibleW: +Math.max(0, vis.r - vis.l).toFixed(1), hit };
  });
};
const restOk = (rs) => rs.every((x) => (Math.abs(x.d) >= 2) === x.far && (x.far ? x.op === 0 && x.pe === "none" && (!x.hit || !x.hit.inSlot) : x.op === 1 && x.pe !== "none") && x.tr === "");
function analyzeWrap(w) {
  const { prevOn, prevD, rest, sync, frames } = w;
  const newD = sync.map((x) => x.d);
  const wrapSlots = newD.map((d, i) => (prevD[i] !== null && Math.abs(d - prevD[i]) > 1 ? i : -1)).filter((i) => i >= 0);
  const newOn = sync.findIndex((x) => x.on);
  const trace = (i) => { const k = frames.findIndex((f) => f.s[i].tr === ""); return { i, dFrom: prevD[i], dTo: newD[i], sync: sync[i].tr, frame0: frames[0] ? frames[0].s[i].tr : null, restoredAtFrame: k, restoredAtMs: k >= 0 ? frames[k].t : null, last: frames.at(-1) ? frames.at(-1).s[i].tr : null, frame0Pos: frames[0] ? [frames[0].s[i].l, frames[0].s[i].r] : null, frame0Op: frames[0] ? frames[0].s[i].op : null }; };
  const wraps = wrapSlots.map(trace);
  const nonWrapNone = sync.map((x, i) => i).filter((i) => !wrapSlots.includes(i) && (sync[i].tr === "none" || frames.some((f) => f.s[i].tr === "none")));
  // 교차 = 측면(data-on 아님) 이고 나가는 활성(prevOn) 이 아니고 보이는(opacity > 0.001) 슬롯이 활성 카드 정지 폭(rest.l..rest.r) 과 0.5px 넘게 겹치는 프레임
  const crossings = [];
  let maxOverlap = 0;
  for (const f of frames) for (const x of f.s) {
    if (x.on || x.i === prevOn || !(x.op > 0.001)) continue;
    const ov = Math.min(x.r, rest.r) - Math.max(x.l, rest.l);
    if (ov > maxOverlap) maxOverlap = ov;
    if (ov > 0.5) crossings.push({ t: f.t, i: x.i, d: x.d, l: x.l, r: x.r, op: x.op, overlapPx: +ov.toFixed(1) });
  }
  const at = (ms) => { if (!frames.length) return null; const f = frames.reduce((a, b) => (Math.abs(b.t - ms) < Math.abs(a.t - ms) ? b : a)); return { t: f.t, s: f.s.map((x) => ({ i: x.i, d: x.d, on: x.on, far: x.far, l: x.l, r: x.r, op: x.op, tr: x.tr })) }; };
  const samples = Object.fromEntries([0, 100, 200, 300].map((ms) => [ms, at(ms)]));
  const sampleCross = Object.fromEntries(Object.entries(samples).map(([ms, s]) => [ms, s ? s.s.filter((x) => !x.on && x.i !== prevOn && x.op > 0.001 && Math.min(x.r, rest.r) - Math.max(x.l, rest.l) > 0.5).length : null]));
  return { prevOn, newOn, prevD, newD, wrapSlots, wraps, nonWrapNone, frames: frames.length, lastT: frames.at(-1) ? frames.at(-1).t : null, crossings, maxSideOverlapPx: +maxOverlap.toFixed(1), samples, sampleCross,
    wrapOk: wraps.every((x) => x.sync === "none" && x.frame0 === "none" && x.restoredAtFrame > 0 && x.last === ""), noCross: crossings.length === 0, nonWrapClean: nonWrapNone.length === 0 };
}
async function wrapPart() {
  const out = {};
  for (const N of [3, 4]) for (const [w, h] of [[1920, 1080], [1440, 900]]) for (const reduce of [false, true]) {
    if (stopped()) { out.stopped = true; break; }
    const key = `n${N}_${w}x${h}${reduce ? "_reduce" : ""}`;
    const { ctx, page, errors } = await open(w, { h, reduce, touch: false, notices: N === 3 ? NOTICE3 : { items: [] }, init: N === 4 ? CLONE4 : null });
    await page.mouse.move(2, 2);
    const n = await page.$$eval(".pdim .pslot", (s) => s.length);
    const rest0 = await page.evaluate(WRAP_REST);
    const r = { n, rest0, rest0Ok: restOk(rest0), steps: [], errors };
    if (N === 4 && w === 1920 && !reduce) { const p = path.join(SHOTS, "popnav_n4_1920.png"); await page.screenshot({ path: p }); shots.add(p); r.shot = p; }
    for (const [k, times] of [["ArrowRight", N], ["ArrowLeft", N]]) {
      for (let j = 0; j < times; j++) {
        await page.evaluate(WRAP_ARM);
        await page.keyboard.press(k);
        await page.waitForTimeout(700);
        const wd = await page.evaluate(() => window.__wrap);
        const a = wd.sync ? analyzeWrap(wd) : { error: "no sync" };
        const rest = await page.evaluate(WRAP_REST);
        const cur = await page.evaluate(() => +document.querySelector(".pdim [data-ppop-cur]").textContent);
        r.steps.push({ key: k, j, cur, ...a, rest, restOk: restOk(rest) });
      }
    }
    const S = r.steps;
    r.v = { n: n === N, rest0: r.rest0Ok, restAll: S.every((s) => s.restOk), noCross: S.every((s) => s.noCross), wrapNoneThenRestored: S.every((s) => s.wrapOk), nonWrapNeverNone: S.every((s) => s.nonWrapClean),
      cycle: S.slice(0, N).at(-1).cur === 1 && S.at(-1).cur === 1, farPresent: N === 4 ? S.every((s) => s.rest.filter((x) => x.far).length === 1) : S.every((s) => s.rest.every((x) => !x.far)), noErrors: errors.length === 0 };
    r.pass = Object.values(r.v).every(Boolean);
    r.summary = { wrapsPerStep: S.map((s) => `${s.key[5]}${s.j}:[${s.wrapSlots}]`).join(" "), crossings: S.reduce((a, s) => a + s.crossings.length, 0), maxSideOverlapPx: Math.max(...S.map((s) => s.maxSideOverlapPx)),
      restoredAtMs: S.flatMap((s) => s.wraps.map((x) => x.restoredAtMs)), frames: S.map((s) => s.frames), farSlots: S.map((s) => s.rest.filter((x) => x.far).map((x) => `${x.i}(d${x.d},op${x.op},pe ${x.pe},vis${x.visibleW}${x.hit ? ",hit " + x.hit.el : ""})`).join("")) };
    out[key] = r;
    log(`wrap ${key}`, r.pass ? "PASS" : "FAIL", JSON.stringify(r.summary), Object.entries(r.v).filter(([, x]) => !x).map(([k2]) => k2).join(","));
    await ctx.close();
  }
  const all = Object.entries(out).filter(([, x]) => x && x.steps);
  out.summary = { runs: all.length, pass: all.filter(([, x]) => x.pass).length, fail: all.filter(([, x]) => !x.pass).map(([k]) => k), steps: all.reduce((a, [, x]) => a + x.steps.length, 0),
    wrapSteps: all.reduce((a, [, x]) => a + x.steps.filter((s) => s.wrapSlots.length).length, 0), wrapSlotEvents: all.reduce((a, [, x]) => a + x.steps.reduce((b, s) => b + s.wrapSlots.length, 0), 0),
    crossings: all.reduce((a, [, x]) => a + x.summary.crossings, 0), maxSideOverlapPx: Math.max(...all.map(([, x]) => x.summary.maxSideOverlapPx)), framesTotal: all.reduce((a, [, x]) => a + x.steps.reduce((b, s) => b + s.frames, 0), 0) };
  log("wrap summary", JSON.stringify(out.summary));
  result.wrap = out; save();
}

// ───────── 10 초점 틀 키 스크롤 (7회차 신규) ─────────
const KS = () => {
  const root = document.querySelector(".pdim");
  if (!root) return { open: false };
  const on = root.querySelector(".pslot[data-on] > .pop"), ae = document.activeElement, t = root.querySelector(".pslot[data-on] h2");
  const d = (e) => (e ? (e === root ? "ROOT" : e.tagName.toLowerCase() + (typeof e.className === "string" && e.className ? "." + e.className.trim().split(/\s+/)[0] : "") + (e.getAttribute("aria-label") ? "[" + e.getAttribute("aria-label") + "]" : "") + "«" + (e.textContent || "").trim().slice(0, 10) + "»") : null);
  return { open: true, cur: +(root.querySelector("[data-ppop-cur]") || { textContent: 1 }).textContent, st: on.scrollTop, sh: on.scrollHeight, ch: on.clientHeight, max: on.scrollHeight - on.clientHeight, overflows: on.scrollHeight > on.clientHeight,
    ae: d(ae), isRoot: ae === root, isTitle: !!t && ae === t, inActive: !!ae && root.querySelector(".pslot[data-on]").contains(ae), scrollY, url: location.pathname + location.hash, pause: (root.querySelector("[data-ppop-pause]") || {}).textContent };
};
async function keysPart() {
  const out = {};
  const KEYS = [["ArrowDown", 1, false, "0"], ["PageDown", 1, true, "0"], ["Space", 1, true, "0"], ["ArrowUp", -1, false, "max"], ["Shift+Space", -1, true, "max"], ["PageUp", -1, true, "max"]];
  const clicksArm = () => { window.__kclk = []; if (!window.__kclkOn) { window.__kclkOn = true; document.addEventListener("click", (e) => { const t = e.target; window.__kclk.push((t.tagName || "").toLowerCase() + (t.getAttribute && t.getAttribute("aria-label") ? "[" + t.getAttribute("aria-label") + "]" : "") + "«" + (t.textContent || "").trim().slice(0, 10) + "»"); }, true); } };
  const waitOverflow = async (p, limit = 9500) => {   // 활성 카드가 넘치지 않으면 자동 넘김(초점 안 옮김)으로 넘치는 카드가 설 때까지
    const t0 = Date.now(); let s = await p.evaluate(KS); const seen = [s.cur];
    while (!s.overflows && Date.now() - t0 < limit) { await p.waitForTimeout(250); s = await p.evaluate(KS); if (seen.at(-1) !== s.cur) seen.push(s.cur); }
    return { waitedMs: Date.now() - t0, curSeq: seen, st: s };
  };
  // 1440x420, 390x640 = 의뢰 기대 두 화면. 1440x320 = 덧붙인 화면: 스크롤 범위가 85% 한 쪽보다 커서 +48, +85% 가 clamp 없이 보이는지(1440x420 은 범위 129 라 PageDown 이 clamp)
  for (const [w, h, touch] of [[1440, 420, false], [390, 640, true], [1440, 320, false]]) {
    const key = `${w}x${h}`;
    const r = { steps: [] };
    { // A. 첫 초점 .pdim 에서 위아래 키
      const { ctx, page, errors } = await open(w, { h, touch });
      r.load = await page.evaluate(KS);
      if (!r.load.overflows) {   // 넘치지 않는 활성 카드: 키가 아무것도 스크롤하지 않는지(관찰)
        r.noOverflow = [];
        for (const k of ["ArrowDown", "PageDown", "Space"]) {
          await page.evaluate(() => window.__kd.splice(0));
          await page.keyboard.press(k); await page.waitForTimeout(250);
          const s = await page.evaluate(KS), kd = await page.evaluate(() => window.__kd.splice(0));
          const kk = kd.filter((e) => e.key === (k === "Space" ? " " : k));
          r.noOverflow.push({ key: k, st: s.st, max: s.max, isRoot: s.isRoot, scrollY: s.scrollY, cur: s.cur, dp: kk[0] ? kk[0].dp : null });
        }
        r.waitAuto = await waitOverflow(page);
      }
      const s0 = await page.evaluate(KS);
      r.testCard = { cur: s0.cur, sh: s0.sh, ch: s0.ch, max: s0.max, isRoot: s0.isRoot };
      for (const [k, sign, pg, from] of KEYS) {
        const start = from === "0" ? 0 : s0.max;
        await page.evaluate((v) => { document.querySelector(".pdim .pslot[data-on] > .pop").scrollTop = v; window.__kd.splice(0); }, start);
        await page.waitForTimeout(60);
        const b = await page.evaluate(KS);
        await page.keyboard.press(k); await page.waitForTimeout(350);
        const a = await page.evaluate(KS), kd = await page.evaluate(() => window.__kd.splice(0));
        const amt = pg ? Math.round(b.ch * 0.85) : 48;
        const exp = Math.max(0, Math.min(b.max, start + sign * amt));
        const kk = kd.filter((e) => e.key === (k.endsWith("Space") ? " " : k));   // Shift+Space 는 Shift keydown 이 먼저 온다. 누른 키 자신의 기록만 본다
        const step = { key: k, start: b.st, amt: sign * amt, exp, st: a.st, delta: a.st - b.st, isRoot: a.isRoot, cur: a.cur, curBefore: b.cur, kdN: kk.length, dp: kk[0] ? kk[0].dp : null, shift: kk[0] ? kk[0].shift : null, kdTgtRoot: kk[0] ? kk[0].tgtIsRoot : null, scrollY: a.scrollY };
        step.ok = Math.abs(a.st - exp) <= 1 && a.st !== b.st && a.isRoot && a.cur === b.cur && step.dp === true && a.scrollY === b.scrollY;
        r.steps.push(step);
      }
      // ArrowRight/ArrowLeft 넘김(초점 .pdim → 새 활성 제목)
      await page.evaluate(() => { document.querySelector(".pdim .pslot[data-on] > .pop").scrollTop = 0; });
      const bR = await page.evaluate(KS); await page.keyboard.press("ArrowRight"); await page.waitForTimeout(700); const aR = await page.evaluate(KS);
      await page.keyboard.press("ArrowLeft"); await page.waitForTimeout(700); const aL = await page.evaluate(KS);
      r.arrows = { from: bR.cur, fromRoot: bR.isRoot, afterRight: aR.cur, rightFocusTitle: aR.isTitle, afterLeft: aL.cur, leftFocusTitle: aL.isTitle, pause: aL.pause };
      r.arrowsOk = aR.cur === (bR.cur % 2) + 1 && aR.isTitle && aL.cur === bR.cur && aL.isTitle && bR.isRoot;
      r.errorsA = errors;
      await ctx.close();
    }
    { // B. Tab 으로 카드 안 조작 요소 → Space, 이어서 첫 <button> → Space
      const { ctx, page, errors } = await open(w, { h, touch });
      await page.evaluate(clicksArm);
      if (!(await page.evaluate(KS)).overflows) r.tabWaitAuto = await waitOverflow(page);
      await page.keyboard.press("Tab"); await page.waitForTimeout(100);
      const t1 = await page.evaluate(KS);
      await page.evaluate(() => { document.querySelector(".pdim .pslot[data-on] > .pop").scrollTop = 0; window.__kd.splice(0); window.__kclk.splice(0); });
      const b1 = await page.evaluate(KS);
      await page.keyboard.press("Space"); await page.waitForTimeout(400);
      const a1 = await page.evaluate(KS), kd1 = await page.evaluate(() => window.__kd.splice(0)), c1 = await page.evaluate(() => window.__kclk.splice(0));
      const k1 = kd1.filter((e) => e.key === " ");
      r.tabLink = { focus: t1.ae, inActive: t1.inActive, cardOverflows: b1.overflows, stBefore: b1.st, stAfter: a1.st, dp: k1[0] ? k1[0].dp : null, kdTgt: k1[0] ? k1[0].tgt : null, clicks: c1, cur: [b1.cur, a1.cur], url: [b1.url, a1.url], open: a1.open, pause: a1.pause, focusAfter: a1.ae };
      let tries = 0, tb = await page.evaluate(KS);
      while (!/^button/.test(tb.ae || "") && tries < 12) { await page.keyboard.press("Tab"); tries++; tb = await page.evaluate(KS); }
      await page.evaluate(() => { document.querySelector(".pdim .pslot[data-on] > .pop").scrollTop = 0; window.__kd.splice(0); window.__kclk.splice(0); window.__prevPop = document.querySelector(".pdim .pslot[data-on] > .pop"); });
      const b2 = await page.evaluate(KS);
      await page.keyboard.press("Space"); await page.waitForTimeout(700);
      const a2 = await page.evaluate(KS), kd2 = await page.evaluate(() => window.__kd.splice(0)), c2 = await page.evaluate(() => window.__kclk.splice(0));
      const prevPopSt = await page.evaluate(() => window.__prevPop.scrollTop);
      const k2 = kd2.filter((e) => e.key === " ");
      r.tabButton = { tabs: tries + 1, focus: b2.ae, prevCardOverflows: b2.overflows, prevCardStBefore: b2.st, prevCardStAfter: prevPopSt, dp: k2[0] ? k2[0].dp : null, kdTgt: k2[0] ? k2[0].tgt : null, clicks: c2, cur: [b2.cur, a2.cur], focusAfter: a2.ae, focusTitle: a2.isTitle, open: a2.open };
      r.tabOk = { linkNotIntercepted: r.tabLink.dp === false && r.tabLink.open, buttonPressed: r.tabButton.dp === false && c2.length === 1 && /^button/.test(b2.ae) && c2[0].startsWith("button") && a2.cur !== b2.cur && a2.isTitle, buttonNoScroll: prevPopSt === b2.st };
      r.errorsB = errors;
      await ctx.close();
    }
    r.v = { loadRoot: r.load.isRoot, keysScroll: r.steps.every((s) => s.ok), noOverflowNoop: !r.noOverflow || r.noOverflow.every((s) => s.st === 0 && s.isRoot && s.scrollY === 0), arrows: r.arrowsOk, ...r.tabOk, noErrors: r.errorsA.length === 0 && r.errorsB.length === 0 };
    r.pass = Object.values(r.v).every(Boolean);
    out[key] = r;
    log(`keys ${key}`, r.pass ? "PASS" : "FAIL", JSON.stringify({ load: [r.load.cur, r.load.ae, r.load.overflows, r.load.sh, r.load.ch], noOverflow: r.noOverflow, waitAuto: r.waitAuto && [r.waitAuto.waitedMs, r.waitAuto.curSeq], card: r.testCard, steps: r.steps.map((s) => `${s.key} ${s.start}->${s.st} (exp ${s.exp}, amt ${s.amt}) root=${s.isRoot} dp=${s.dp} cur=${s.curBefore}->${s.cur}`), arrows: r.arrows, tabLink: r.tabLink, tabButton: r.tabButton }), Object.entries(r.v).filter(([, x]) => !x).map(([k]) => k).join(","));
  }
  out.pass = Object.values(out).filter((x) => x && x.v).every((x) => x.pass);
  result.keys = out; save();
}

const parts = { nav: navPart, seq: seqPart, sweep: sweepPart, restpos: restposPart, pause: pausePart, n1: n1Part, visual: visualPart, close: closePart, focus: focusPart, short: shortPart, wrap: wrapPart, keys: keysPart };
