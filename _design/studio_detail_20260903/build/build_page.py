...
"""
import base64, io, json, pathlib, sys, datetime, html
...
def img(uri_size, alt, cls='', lazy=True):
    ...
    return f'<img class="{cls}" src="{uri}" width="{w}" height="{h}" alt="{alt}"{lz} decoding="async">'
...
def unit_card(u):
    ...
    return (f'<li class="unit rv"><span class="mono">{esc(u["label"].split()[0])}</span><h4>{esc(title)}</h4><p class="sub">{esc(sub)}</p>'
...
<section class="hero" id="top">
  <div class="wrap">
    ...
    <div class="stage" aria-label="스튜디오 실제 화면 세 장">
      {{STAGE}}
    </div>
  </div>
</section>
  ...
  <div class="wrap">
    <div class="split one">
      <div class="txt rv">
        ...
        <ul class="forms">
          <li><span class="mono">읽는 순서</span><h4>“네 편 요약은 다 했는데, 뭘 물었는지는 답을 못 했어요.”</h4><p>요약은 답이 아닙니다. 발문이 지목한 기준으로 판단해야 답이 됩니다.</p></li>
          <li><span class="mono">쓰는 분량</span><h4>“문장으로 쓰다가 준비 시간이 끝났어요.”</h4><p>세 문항 발화 분량은 2,000자에 가깝습니다. 21분 안에 손으로 쓸 수 없는 분량.</p></li>
          <li><span class="mono">시간 감각</span><h4>“시간이 남아서 했던 말을 또 했어요.”</h4><p>닫는 문장을 정해 두지 않으면 마지막 30초가 늘어집니다.</p></li>
...
<section class="sec" id="who">
  <div class="wrap">
    ...
    <ul class="aud">
      <li class="rv"><span class="mono">학생</span><h4>연세대 활동우수형과 국제형, 고려대 계열적합전형 지원자</h4><p>지원 단위의 기출 규격으로 응시 기록을 쌓는 학생. 실전형 한 번 뒤 연습형으로 막힌 문항만 다시 하고 실전형으로 확인.</p></li>
      <li class="rv"><span class="mono">학부모</span><h4>준비 상태를 리포트로 확인하려는 학부모</h4><p>응시마다 여섯 축 점수와 첨삭이 마이페이지에 남습니다. 어느 축이 낮은지와 다음 회차에 무엇이 올라갔는지가 숫자로 보입니다.</p></li>
      <li class="rv"><span class="mono">학원, 학교</span><h4>여러 학생을 같은 기준으로 지도하는 곳</h4><p>강사 화면에서 전 시도의 영상과 전사와 첨삭을 열람합니다. 좌석 단위 도입은 스쿨 플랜으로.</p></li>
    </ul>
  </div>
</section>
...
<section class="sec mat" id="trust">
  <div class="wrap">
    ...
    <ul class="pr rv">
      <li><span class="mono">원칙 1</span><h4>기출 규격 실측</h4><p>연세대 2026학년도 문항카드와 고려대 3개년 문항카드 12장을 전수 실측했습니다. 제시문 편수와 문항 수와 배점과 시간을 그대로 옮겼습니다.</p></li>
      <li><span class="mono">원칙 2</span><h4>세트 전수 정합 판정</h4><p>150세트 전부를 기출 유형 비율과 배점 구조에 대조해 판정합니다. 판정을 통과한 세트만 스튜디오에 올립니다.</p></li>
      <li><span class="mono">원칙 3</span><h4>루브릭 대조 채점</h4><p>기준마다 모범 포인트를 대조하고 전사문을 인용한 뒤 다섯 밴드로 판정합니다. 경계선은 낮은 밴드로 둡니다.</p></li>
      <li><span class="mono">원칙 4</span><h4>판을 밝힘</h4><p>세트마다 난이도와 네 축 점수를 공개합니다. 풀이법 인강 공개 편수는 열람 시점 기준으로 적습니다.</p></li>
    </ul>
    <div class="quote rv">
...
<section class="part" id="p1">
  <div class="wrap">
    ...
    <div class="body">
      <div class="vis rv"><div class="shot">{{IMG_HOME}}</div></div>
      <div class="txt rv">
    ...
    <div class="sample rv">
          ...
          <table class="setlist">
            <tr><th>단위</th><td>세트 제목</td><td class="r">난이도</td></tr>
            {{SETROWS}}
          </table>
    </div>
  </div>
...
<section class="part rev" id="p2">
  <div class="wrap">
    ...
    <div class="body">
      <div class="vis rv"><div class="shot">{{IMG_PREP}}</div></div>
      <div class="txt rv">
        <div class="blk"><h3>이 단계에 들어 있는 것</h3>
          <ul>
    ...
    <div class="sample rv">
          ...
          <div class="cols">
            <table>
              <tr><th>연세대 활동우수형, 국제형</th><td></td></tr>
              <tr><th>준비 시간</th><td>480초, 8분</td></tr>
              <tr><th>답변 시간</th><td>300초, 5분</td></tr>
              <tr><th>구성</th><td>제시문 4편, 문제 2개</td></tr>
              <tr><th>배점</th><td>문제 1과 문제 2, 60점과 40점</td></tr>
            </table>
            <table>
              <tr><th>고려대 계열적합전형</th><td></td></tr>
              <tr><th>준비 시간</th><td>1,260초, 21분</td></tr>
              <tr><th>발화 시간</th><td>420초, 7분 통합</td></tr>
              <tr><th>구성</th><td>제시문 여러 편, 3문항. 자연은 뒤 문항이 앞 문항의 개념을 참조</td></tr>
              <tr><th>배분</th><td>문항당 140초 기준, 실전형은 스스로 배분</td></tr>
            </table>
          </div>
    </div>
  </div>
...
<section class="part" id="p3">
  <div class="wrap">
    ...
    <div class="body">
      <div class="vis rv"><div class="shot">{{IMG_ANSWER}}</div></div>
      <div class="txt rv">
        <div class="blk"><h3>이 단계에 들어 있는 것</h3>
          <ul>
...
<section class="part core rev" id="p4">
  <div class="wrap">
    ...
    <div class="body">
      <div class="vis rv"><div class="shot">{{IMG_REVISION}}</div></div>
      <div class="txt rv">
        <div class="blk"><h3>이 단계에 들어 있는 것</h3>
          <ul>
...
<section class="part" id="p5">
  <div class="wrap">
    ...
    <div class="body">
      <div class="vis rv"><div class="shot">{{IMG_REPORT}}</div></div>
      <div class="txt rv">
        <div class="blk"><h3>이 단계에 들어 있는 것</h3>
          <ul>
    ...
    <div class="sample rv">
          ...
          <table class="ladder">
            <tr><th>1회</th><td>실전형</td><td>통합 답변으로 시작 상태를 잽니다. 리포트의 낮은 축이 기준선.</td></tr>
            <tr><th>2회</th><td>연습형</td><td>진단이 인용한 문항만 따로. 구술체 재구성을 따라 말한 뒤 응시.</td></tr>
            <tr><th>3회</th><td>연습형</td><td>다른 문항. 같은 유형의 진단이 다시 뜨는지 봅니다.</td></tr>
            <tr><th>4회</th><td>연습형</td><td>시간 배분. 문항별 답변 시간을 실전 배분에 맞춰 닫는 문장까지.</td></tr>
            <tr><th>5회</th><td>실전형</td><td>통합 답변으로 확인. 1회와 같은 축이 올라갔으면 다음 세트로.</td></tr>
          </table>
    </div>
  </div>
</section>
...
<section class="sec mat" id="diff">
  <div class="wrap">
    ...
    <table class="cmp rv">
      <thead><tr><th>단계</th><th>흔한 준비</th><th>이 스튜디오의 순서</th></tr></thead>
      <tbody>
  ...
  <div class="wrap">
    <div class="split">
      <div class="txt rv">
        ...
        <p class="lede" style="margin-top:var(--s4)">카메라와 마이크가 있는 브라우저면 됩니다. 응시 시간은 대학 규격과 같고 회차마다 세 단 첨삭이 마이페이지에 붙습니다. 응시 시작 전에는 전액 취소할 수 있습니다.</p>
        <ol class="flow">
          <li><span class="n">01</span><h4>회원 가입과 체험 응시</h4><p>회원은 무료 응시 1회. 실제 세트로 리포트까지 받습니다.</p></li>
          <li><span class="n">02</span><h4>지문 또는 단위 고르기</h4><p>지문 낱권 한 편, 지원 단위 전권 30편, 또는 공통 풀이 인강만.</p></li>
          <li><span class="n">03</span><h4>마이페이지에서 응시</h4><p>스튜디오로 바로 열립니다. 리포트와 영상은 마이페이지에 보관.</p></li>
        </ol>
      </div>
    ...
    <ul class="price rv">
      ...
      <li><h4>스쿨 플랜</h4><div class="won two">좌석 5개<small>부터, 문의</small></div>
...
<section class="sec" id="faq">
  <div class="wrap">
    ...
    <div class="faq rv">
      ...
      <details><summary>환불은</summary><div class="a">응시 시작 전 전액. 시작 이후는 약관의 환불 조항을 따릅니다.</div></details>
...
def main():
    stage = ''.join(f'<div class="scr s{i}">{img(STAGE[k], alt, "", lazy=False)}</div>'
                    for i, (k, alt) in enumerate([('report', '평가 리포트 화면'), ('brief', '응시 전 점검 화면'), ('prep', '준비 시간 화면')]))
    ...
    for c in ORDER:
        ...
        for i, s in enumerate(u['sets'][:3]):
            th = f'<th rowspan="3">{esc(UNIT_COPY[c][0])}</th>' if i == 0 else ''
    ...
    h = (h
          ...
          .replace('{{IMG_REVISION}}', img(IMG['revision'], '첨삭 화면, 전사록 정리와 진단 여섯 유형'))
          .replace('{{IMG_REPORT}}', img(IMG['report'], '평가 리포트 화면, 여섯 축 도형과 문항별 루브릭 채점'))
