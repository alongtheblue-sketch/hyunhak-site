...
VARIANTS = {
    ...
    # 2026-09-03 s39 건우 지시(각 부 실물 면): 1부·2부·4부(창체) 표본 추가. 좌표 = fitz blocks 실측(pt).
    'D': dict(page=4, title='1부 면접 형태 판정 면', zoom=(42.5, 276.0, 362.0, 334.0), regions=[
        dict(rect=(42.5, 106.0, 516.0, 131.0), label='세 형태 중 서류기반. 정의와 이 대학에 있음 표시'),
        dict(rect=(42.5, 276.0, 362.0, 334.0), label='전형별 형태 판정표. 가천바람개비전형은 서류확인형'),
    ]),
    'E': dict(page=7, title='2부 학과별 인재상 면', zoom=(73.0, 144.0, 240.0, 249.0), regions=[
        dict(rect=(73.7, 42.0, 491.0, 95.0), label='확인 질문에서 역추론한 학과별 초점. 질문 5건 이상 확인된 36개 학과 수록'),
        dict(rect=(73.0, 114.0, 240.0, 249.0), label='간호학과 82건. 확인하는 것과 실제 질문 두 건'),
    ]),
    'F': dict(page=22, title='4부 창의적 체험활동 규칙 면', zoom=(53.2, 66.0, 400.0, 154.0), regions=[
        dict(rect=(42.5, 42.0, 400.0, 154.0), label='영역 이름과 규칙 한 건. 전교 회장, 부회장 기재 때의 질문 틀, 꼬리질문, 평가 축'),
        dict(rect=(53.2, 177.0, 396.0, 264.0), label='영재학급, 심화과정 기재 때의 규칙'),
    ]),
}
