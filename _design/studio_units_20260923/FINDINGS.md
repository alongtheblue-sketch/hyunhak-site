...
## 1. 🔴 라이브 돈 결함 (09-14 f65b096 부터 운영 중)
...
6. 실측: 로컬 8092 에서 12카드 클릭 → 담기 → HH.cart() sku = pass-<해당 코드> 12/12, select option 12, 콘솔 오류 0. 라이브 배포 뒤 같은 검사.
...
- 수리 = D1 원격 쓰기 → **결재 큐 §HH-STUDIO-UNITS-20260923 SU-1**(표적 2행 INSERT OR IGNORE, 형식 tools/qa_seed_master_mirae_20260914.sql:14-17. master_grant_20260828.sql 재실행 금지 = 중복 행과 digital 권리 대량 생성). meta.title 정정은 SU-2.
