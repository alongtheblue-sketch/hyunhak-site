# 8축 코드 교차검토 s3 r1 (2026-09-14 14:1x) — 구매한 회원의 이용 진입 UX, API P0 2건 + 계약 4건 + 약관 개시 시점 + 잔여 패치 7건
...
## 검증 실측
- API: 새 테스트 6건 red(150 pass / 6 fail) → 패치 → green 156/156 (전 스위트 155/156 → 구 summary 테스트 total 4→3 갱신 뒤 lecture.test.js 46/46 재확인). historySetRe 단위검사 6사례 len ≤176, 누락 0, 초과 0. 전 스위트는 배포 스크립트가 다시 돈다.
- 사이트: build_all 게이트 전건 PASS(해시 b6029282d8434e3b), Playwright 4상태×2폭×8면 + 앱 4면 렌더(오류·넘침 프로브) = 별도 리포트.

## 요청
8축(정합성·동시성·보안·경계·회귀·문안·접근성·계약)으로 diff 를 본다. 특히:
1. trial.js batch 의 changes() 결속이 D1 batch(한 연결·한 트랜잭션) 에서 유료 분기와 같은 의미로 성립하는가. 두 병렬 redeem(다른 ref) 에서 원장 2행이 들어갈 경로가 있는가.
2. pay.js: 멱등 재방문에서 PG 조회가 다른 주문·다른 회원의 계좌를 돌려줄 경로(orderId 소유자 조건, portone getPayment 의 paymentId=orderId) 가 있는가. GET /orders 의 Promise.all 이 한 조회 실패로 전체 500 이 되는가.
3. reader.js·lecture.js 의 ORDER BY (expires_at IS NULL) DESC, expires_at DESC 가 "무기한 우선, 아니면 가장 늦은 만료" 를 SQLite 에서 보장하는가. 만료 code 응답이 미구매자에게 권리 존재를 누설하는가(권리 없는 사람은 code 없음).
4. historySetRe: fullmatch 전제에서 앵커 없는 소스가 안전한가, 200자 상한 fallback 이 fail-open 을 만드는 경우.
5. 사이트: my.html 2단계 버튼이 스크린리더·키보드에서 상태를 알리는가(aria), .hgo 위임과 기존 querySelectorAll 결속의 이중 실행 가능성, owned.js 다편 목록에서 goBtn(e,'btn ghost sm') 의 data-owned-go 결속(bind) 이 새 ul 에 미치는가.
6. 약관 문면: "응시를 시작한 때 또는 세트 문제지나 해설지 PDF 가 발급된 때" 가 제6조 다른 항(전권 묶음)과 모순 없이 읽히는가.

출력: [축][real|nit] SEVERITY(HIGH/MEDIUM/LOW) 파일:행 — 한 줄 결함 + 한 줄 수정. real 만 세고 nit 는 뒤에. 마지막 줄 "VERDICT: GO|NO-GO" 와 근거 1줄. 한국어, 20줄 상한.

## diff — hyunhak-api (src/pay.js, src/trial.js, src/reader.js, src/lecture.js)
```diff
...
index ea36e83..12aff30 100644
--- a/src/trial.js
+++ b/src/trial.js
@@ -20,6 +20,34 @@ export const TRIAL_SET_IDS = Object.freeze([
...
+export function historySetRe(res, { maxLen = 200 } = {}) {
...
+    for (const [p, set] of nums) {
+      const pat = set.size >= minFull ? SET_NO_ALL : `(${[...set].sort().join("|")})`;
+      if (!groups.has(pat)) groups.set(pat, []);
+      groups.get(pat).push(p);
+    }
+    return [...groups].map(([pat, ps]) => `${ps.length > 1 ? `(${ps.join("|")})` : ps[0]}${pat}`).join("|");
+  };
+  let src = build(30);
+  if (src.length > maxLen) src = build(15);   // 접두어 절반 이상 보유 = 그 단위 전부로 넓힌다 (상한 안에서 넓힘 최소)
+  if (src.length > maxLen) src = build(1);    // 그래도 길면 보유 접두어 전부 (7단위 전부여도 130자 안)
+  return src;
+}
+
...
@@ -57,8 +85,14 @@ trial.post("/studio/token", requireMember(async (c) => {
...
@@ -157,18 +191,22 @@ trial.post("/trial/redeem", async (c) => {
   if (p.kind === "trial") {
...
index 3eddfcd..9feee19 100644
--- a/assets/base.css
+++ b/assets/base.css
@@ -1505,6 +1505,12 @@ html.ppop-open{overflow:hidden}
...
+        var ul=document.createElement('ul'); ul.className='owned-picks';
...
@@ -321,7 +322,7 @@ dialog.rf::backdrop{background:rgba(49,46,46,.42)}
...
@@ -357,7 +358,7 @@ dialog.rf::backdrop{background:rgba(49,46,46,.42)}
...
+    <nav class="ft-legal" aria-label="법적 고지"><a href="support.html">고객센터</a><a href="#business-info">사업자 정보</a><a href="faq.html#q-pay">결제</a><a href="terms.html">환불 규정</a><a href="terms.html">이용약관</a><a href="privacy.html">개인정보처리방침</a></nav>
     ...
     <div class="g">
@@ -375,7 +376,7 @@ dialog.rf::backdrop{background:rgba(49,46,46,.42)}
...
@@ -485,13 +486,15 @@ document.addEventListener('DOMContentLoaded', async function(){
...
@@ -522,13 +525,14 @@ document.addEventListener('DOMContentLoaded', async function(){
...
+      var did=n>0, left=Math.max(0,SET_USES-n), armAttr=arm?' data-sdl-arm="1"':'';
...
@@ -552,24 +556,25 @@ document.addEventListener('DOMContentLoaded', async function(){
...
+        var arm=!noLedger&&!expired&&didIds.length===0;   // 응시 0회 전권 = 첫 PDF 발급이 제공 개시
...
+            : '<p class="sr-h">아직 응시한 세트가 없습니다. 응시하러 가기로 시작하면 제시문과 문제가 앱 화면에 나옵니다. 세트마다 '+SET_USES+'회 응시할 수 있고, 아래 문제지와 해설지 PDF 는 응시한 뒤 복습용입니다. 응시 전에 발급받으면 이 주문의 청약철회가 제한됩니다(이용약관 제6조)</p>');
...
+        if(rowsEl&&HH.okSetId(sid)) rowsEl.innerHTML=setRow(sid,nOf[sid]||0,expired,!noLedger&&!expired&&!(nOf[sid]>0));
       }
     });
     // 응시 기록 표 (최신순, 원장 그대로)
@@ -604,13 +609,17 @@ document.addEventListener('DOMContentLoaded', async function(){
...
+  document.getElementById('stuRows').addEventListener('click', function(ev){ var b=ev.target.closest('.hgo'); if(!b) return; ev.preventDefault(); HH.studioGo(b); });
   ...
   (async function(){
@@ -707,6 +716,13 @@ document.addEventListener('DOMContentLoaded', async function(){
...
+    if(b.dataset.sdlArm==='1'&&b.dataset.sdlArmed!=='1'){
+      b.dataset.sdlArmed='1'; b.textContent=SDL_DOC[doc]+' PDF, 제한에 동의하고 받기';
+      var row=b.closest('.r');
+      if(row&&!row.querySelector('.sr-warn')){ var w=document.createElement('p'); w.className='sr-warn'; w.textContent='아직 응시하지 않은 이용권입니다. 문제지나 해설지 PDF 를 발급받으면 제공이 개시되어 이 주문의 청약철회가 제한됩니다(이용약관 제6조). 받으려면 버튼을 한 번 더 누르세요.'; row.appendChild(w); }
+      return;
+    }
...
@@ -753,6 +769,8 @@ document.addEventListener('DOMContentLoaded', async function(){
