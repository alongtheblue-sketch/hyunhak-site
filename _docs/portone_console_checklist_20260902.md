...
| 20 | 신청: KG 신용카드 일반결제 | onboarding | 바로오픈으로 실결제 개통(09-03). 카드사 심사는 KG 본계약(전자계약) 뒤 진행 | 진행중 | 9/30 전 본계약, 심사 요청 |
| 21 | 신청: KG 간편결제 | onboarding | PG사 접수 완료. 바로오픈 미지원 | 진행중 | 심사 후 10월 오픈 예정 문구와 함께 재개 |
| 22 | 신청: NHN KCP 5건 | onboarding | 전건 철회 완료 (pgs@portone.io 09-02 14:29 회신, 09-03 실측) | 완료 | PC-2 닫힘. 콘솔 배너가 남아 있어도 진행 금지 |
| 23 | 바로오픈 | help open-immediately, cs@portone 09-01 14:52 메일 | MID MOI0126978, 가계약 9/1~9/30, 카드 일시불만. **실결제 1건 + 전액 환불로 개통 검증 완료(09-03 18:43/18:47)**. 초기등록비 납부 여부는 건우 KG 화면 확인 몫 | 완료 | 9/30 전 본계약(전자계약, 카드사 심사 요청) |
| 24 | 결제창 필수 파라미터 | pg/v2/inicis-v2 | customer fullName + PC 는 phoneNumber, email 필수 → 코드 반영(cc4ed79), s3 실브라우저 검증 | 완료 | 없음 |
| 25 | 정산 계좌, 세금계산서 | 콘솔 | 별도 메뉴 없음(PG 계약 단계). KG 관리자 정산정보 메뉴 존재 | 대기 | KG 전자계약 시 |

## 집행 기록
- 09-03 18:47:22 **전액 환불 (건우, api.hyunhak.com/admin 주문 취소)**: 콘솔 = 전체취소, 전체 취소 금액 33,000 KRW, 웹훅 발송 성공. 결제완료 0, 결제취소 1.
- 09-03 18:43:07 **실결제 1건 (건우, 테스트 회원 cc-portone-test)**: 콘솔 통합 결제 내역 = 결제완료, 고객사 거래번호 ord_fb3368cf1a23, KG이니시스 카드결제 33,000 KRW, 주문명 "가천대학교 2027 면접 가이드북", 웹훅 상태 발송 성공, 결제 모드 실결제/일반. 환불은 별도 결재 후.
- 09-03 18:4x 문구 수정 배포(건우 `!`, detached worktree c54bde8, 타 세션 미커밋 60파일 격리). 버전 3b7adfe5, 자산 2 갱신(checkout, faq), 스모크 16/16. 라이브 실측 = "10월 중 오픈 예정" checkout 1·faq 2, METHODS CARD, /.git 404, 푸터 "면제대상"(전 페이지 동일).
