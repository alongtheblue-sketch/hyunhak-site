# 8축 코드 교차검토 발주 (2026-09-13, hyunhak 구매물 활용 UX)

# Codex Cross-Check — 코드 리뷰 외부 시선

당신은 코드의 *외부 리뷰어*입니다. 내부(code-architect + dt-qa)와는 독립적으로 보고 다른 관점·놓친 결함 발견에 집중하세요.
...
| # | 축 | 핵심 |
|---|---|---|
| 1 | 정확성 (correctness) | 의도한 동작·edge case |
| 2 | 보안 (OWASP Top 10) | injection·XSS·secrets·path traversal |
| 3 | 성능 (Big-O·메모리) | 명백한 quadratic·메모리 누수 |
| 4 | 동시성 | race·deadlock·atomicity |
| 5 | 에러 처리 | 시스템 경계만·내부 trust |
| 6 | 가독성·명명 | 의도 드러나는 이름 |
| 7 | 테스트 가능성 | mock 가능·boundary 분리 |
| 8 | 의존성·라이선스 | 신규 lib 도입 시 SCA·라이선스 |
...
## 절대 룰
- 시크릿 노출 발견 = 자동 BLOCK_MERGE
- 추측 금지. 코드 직접 인용 + 파일:줄 명시
- 본 세션 PM 이 종합 결정 — Codex 권고는 input 일 뿐
- 한국어 우선, 코드 인용은 그대로 (번역 X)


## 맥락
hyunhak.com (정적 사이트, Cloudflare Worker API api.hyunhak.com). 학생 피드백: 구매 뒤 스튜디오 탭이 판매 면(programs/studio.html)으로 열려 산 단위에 "구매하러 가기" 가 다시 보이고, 응시 진입은 MY 의 "응시하러 가기" 하나뿐이라 못 찾음. 인강 위치도 못 찾음.
수리: 공용 모듈 assets/owned.js 신설 — HH.me().entitlements(/api/auth/me) 를 읽어 (1) 판매 면·앱 면·홈·자료실·인강 면 상단에 "내가 산 것" 블록(상품 1줄 = 버튼 1개) (2) 산 단위 카드의 구매 CTA 를 "응시하러 가기" 로 교체 (3) 자료실 산 권 "보유 중" (4) 응시 진입 HH.studioGo = POST /api/studio/token 뒤 휴대폰은 같은 탭 location.assign, 데스크톱은 window.open. my.html 의 .sgo 도 같은 함수로 통일. interview-studio 앱은 "마이페이지" 되돌아가기 링크와 홈 안내 1문장.
서버 계약(불변): /api/auth/me → {member, trial_available, entitlements:[{id,kind,meta(JSON 문자열),uses_left,expires_at}]}, kind ∈ studio_passage|studio_school|download|file_download|lecture|trial. studio_school meta = {sku:'pass-<unit>', unit_code?, title?}, studio_passage meta = {set_id, title?}. POST /api/studio/token body {set_id?, entitlement_id?} → {url, scope}. 서버 trial.js 는 미만료 + (전권 또는 uses_left>0) 권리만 연다.
검토 범위: 아래 diff 전부 + owned.js 전문. 특히 (a) XSS/인젝션(innerHTML 에 들어가는 값 전부 esc 통과 여부) (b) 상태 분기 누락(비회원·네트워크 오류·만료·잔여 0·낱권만 보유·전권+낱권 중복·9/14 개시 단위) (c) 기존 핸들러 충돌(product_buy.js data-r3-unit-buy 위임, studio.html data-r3-unit-cart 위임, my.html ENT_ID_RE 잔존) (d) 모바일 팝업 차단과 location.assign 전환의 부작용 (e) 회귀(비보유 회원·비회원 화면 무변화) (f) 접근성(aria-busy, hidden, 버튼 vs 링크) (g) 성능(me 캐시, sets.json 미사용). 
출력: 지적 각 1줄 = [축][severity real|nit|false-positive 후보] 파일:위치 — 문제 — 수정안. 마지막 줄 = "VERDICT: GO|NO-GO" 와 한 줄 근거. 한국어. 45줄 상한.
...
(function(){
  ...
  async function owned(force){
    ...
    (st.entitlements||[]).forEach(function(e){
      ...
      if(e.kind==='lecture'){ o.lecture.push(e); return; }
  ...
  async function studioGo(el){
    ...
    try{
      ...
      if(coarse){ location.assign(r.url); return; }
      var w=window.open(r.url,'_blank','noopener');
      if(w===false) location.assign(r.url);
    }catch(e){ alert((e&&e.message)||'이용권을 확인하지 못했습니다'); }
  ...
  function items(o, pre){
    ...
    o.guide.singles.forEach(function(e){
      var op=e.kind==='file_download'
        ? '<a class="btn sm" href="'+pre+'my.html">MY에서 내려받기</a>'
        : '<a class="btn sm" href="'+pre+'reader.html?slug='+encodeURIComponent(e._slug)+'">열람하기</a>';
      out.push({nm:esc(e._meta.title||e._slug), st:e.kind==='file_download'?'PDF 소장판':'보안 뷰어 열람', op:op});
    });
    if(o.trial) out.push({nm:'스튜디오 무료 체험', st:'신규 회원 무료 응시 1회', op:'<button type="button" class="btn ghost sm" data-owned-go>체험 응시</button>'});
    return out;
  }
  ...
  async function mount(el){
    ...
    el.innerHTML='<section class="owned" aria-labelledby="ownedT"><div class="owned-h"><h2 id="ownedT">내가 산 것</h2><a class="tlink" href="'+pre+'my.html">MY에서 전체 보기</a></div>'
      +'<ul class="owned-list">'+list.map(function(i){ return '<li><div><p class="nm">'+i.nm+'</p>'+(i.st?'<p class="st">'+esc(i.st)+'</p>':'')+'</div><div class="ops">'+i.op+'</div></li>'; }).join('')+'</ul>'
      +(o.studio.some(live)?'<p class="owned-note">응시 순서: 응시하러 가기를 누르면 스튜디오가 열립니다. 세트를 고르고 면접 시작을 누르면 준비 시간 뒤 답변 녹화가 시작됩니다. 점수와 첨삭은 MY 응시 기록에서 봅니다.</p>':'')
      +'</section>';
    el.hidden=false;
    bind(el);
  }
  ...
  function applyUnits(root){
    ...
    (root||document).querySelectorAll('article[data-r3-unit]:not([data-owned-applied])').forEach(function(card){
      ...
      if(school && buy) buy.replaceWith(go);
      else if(buy) foot.insertBefore(go,buy);
      else foot.appendChild(go);
    });
    bind(root);
  }
...
## hyunhak-site git diff
diff --git a/assets/base.css b/assets/base.css
index 9e3f55c..f0a8a8a 100644
--- a/assets/base.css
+++ b/assets/base.css
@@ -1489,3 +1489,23 @@ html.ppop-open{overflow:hidden}
...
+:where(body.v2) .owned-guest{margin:var(--s3) 0}
+:where(body.v2) .r3-unit-badge.own{color:var(--seal);border-color:currentColor;font-weight:600}
+:where(body.v2) .liblist .lf.own .own-tag{color:var(--seal);font-weight:600}
+@media (max-width:640px){
+  :where(body.v2) .owned{padding:var(--s3)}
+  :where(body.v2) .owned-list li{align-items:stretch;flex-direction:column;gap:var(--s2)}
+  :where(body.v2) .owned-list .ops .btn{width:100%}
+}
...
@@ -596,15 +597,8 @@ document.addEventListener('DOMContentLoaded', async function(){
...
+<div class="wrap"><div data-owned hidden></div></div>
 ...
 <!-- 지면 1. 응시 단위 5. 카드는 assets/data/sets.json 으로 그리고 문안은 COPY.md §3 -->
@@ -339,6 +340,7 @@
 <nav class="fix" aria-label="모바일 바로가기"><a href="index.html"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M4 11.2 12 4.4l8 6.8M6.4 9.8V20h11.2V9.8"/></svg>홈</a><a href="programs/guidebook.html"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><rect x="5" y="4" width="14" height="16"/><path d="M8.6 4v16M12 8.4h4M12 11.8h4"/></svg>가이드북</a><a class="on" aria-current="page" href="programs/studio.html"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><rect x="3" y="6" width="12" height="12" rx="2"/><path d="m15 10 6-3v10l-6-3z"/></svg>스튜디오</a><a href="ranking.html"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M4 20V12h5v8M9 20V5h6v15M15 20v-10h5v10M3 20h18"/></svg>랭킹실</a><a href="my.html"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><circle cx="12" cy="8.4" r="3.4"/><path d="M4.8 20c1.6-4 4.2-5.6 7.2-5.6s5.6 1.6 7.2 5.6"/></svg>MY</a></nav>
...
## interview-studio git diff (web/index.html 만)
diff --git a/web/index.html b/web/index.html
index 2a1193f..2b665b3 100644
--- a/web/index.html
+++ b/web/index.html
@@ -323,6 +323,7 @@
...
@@ -1638,6 +1639,7 @@ async function loadMe(){
