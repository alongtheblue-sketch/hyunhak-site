# 8축 코드 교차검토 s3 r3 (2026-09-14 16:0x) — 구매한 회원의 이용 진입 UX, API P0 2건 + 계약 4건 + 약관 개시 시점 + 잔여 패치 7건
...
## r2 판정과 처분 (당신의 r2 = NO-GO, real 1 + nit 2)
1. [접근성] MEDIUM my.html 같은 행 두 번째 버튼의 aria-describedby 가 없는 id 참조 → **수용 및 수리**: 경고 id 를 문서 종류가 아니라 행 단위로 둔다. 행에 .sr-warn 이 이미 있으면 그 id 를 재사용, 없으면 `sdlw_<set_id>_<일련번호>` 로 새로 만든다(일련번호 = 카드가 달라도 같은 set_id 가 있을 수 있어 세트명만으로는 고유하지 않다). 렌더 프로브: 문제지 클릭 뒤 해설지 클릭 → 두 버튼 describedby 동일 `sdlw_yonsei_2027_h01_1`, 대상 실재, 같은 행, 행의 .sr-warn 1개.
2. [회귀][nit] 테스트에 원래 반례 부재 → **수용**: `astra r1 원래 반례: 4접두어 01~15, 02~16, 03~17, 04~18` 사례 추가, 19~30 과 이웃 접두어 전부 불허 확인. 8/8.
3. [문안][nit] 원장 실패에도 "아직 응시하지 않은" 단정 → **수용**: setRow 가 noLedger 면 `data-sdl-unsure="1"` 을 붙이고, 경고 첫 문장을 "응시 여부를 지금 확인하지 못했습니다. 아직 응시하지 않은 이용권이면 문제지나 해설지 PDF 발급으로 제공이 개시되어 ..." 로 가른다.
4. 주석의 가운뎃점 4곳을 쉼표로 (문체 규칙, 행동 무관).

## 검증 실측 (r3 시점)
- API: history_set_re 8/8, studio_ledger 10/10, lecture 46/46 (r1 뒤 재실행). 전 스위트는 배포 스크립트가 다시 돈다.
- 사이트: build_all 전 게이트 PASS, Playwright 68면 넘침 0, 오류 0(guest 401 은 설계). my.html 프로브 위 1 항.

## 요청
r2 의 1건이 닫혔는지와, 수리가 새로 연 결함이 있는지만 본다. 특히 sdlWarnSeq 일련번호와 prev 재사용의 결합에서 (a) 같은 행을 다시 그릴 때(원장 재조회로 rowsEl.innerHTML 교체) 옛 id 를 가리키는 버튼이 남는가, (b) 두 카드가 같은 set_id 행을 가질 때 id 충돌이 있는가, (c) unsure 문안이 약관 개시 기준과 모순되는가.

출력: [축][real|nit] SEVERITY 파일:행 — 한 줄 결함 + 한 줄 수정. r2 1건 "닫힘/미닫힘" 한 줄. 마지막 줄 "VERDICT: GO|NO-GO" 와 근거 1줄. 한국어, 16줄 상한.

## r2 당신의 판정 원문
```
[접근성][real] MEDIUM `hyunhak-site/my.html:727–728` — 같은 행에서 문제지→해설지 버튼을 차례로 첫 클릭하면, 해설지의 `aria-describedby`가 생성되지 않은 `_key` id를 참조합니다.  
수정: 기존 `.sr-warn.id`를 재사용하고, 이용권·세트 행마다 고유한 id로 두 버튼을 연결합니다.  
r1-1 보안: 닫힘 — 번호 집합을 정확히 표현하며 절단도 허용 범위를 넓히지 않습니다.  
r1-2 경계: 닫힘 — `noLedger`에서도 확인을 유지합니다. 이미 응시한 회원에게 추가 확인을 요구하는 정도는 수용 가능합니다.  
r1-3 접근성: 미닫힘 — 사전 live 영역은 추가됐지만 두 번째 PDF 버튼의 설명 연결이 깨집니다.  
r1-4 계약: 닫힘 — FAQ 본문·JSON-LD, MY 모달, 약관·checkout·고객센터의 응시/PDF 발급 개시 기준이 일치합니다.  
r1-5 문안: 닫힘 — 제한 범위가 낱권은 해당 이용권, 전권은 해당 전권 이용권 전체로 정리됐습니다.  
[보안] `digitRuns`·`0[1-9]`·단일 번호는 정확합니다. `fullmatch`에서 접두어 그룹 누출도 없습니다.  
[보안] 절단은 보유 수가 적은 접두어부터이며, 로그에는 회원 ID와 고정 접두어 외 추가 PII가 없습니다.  
[동시성] 단일 grant 전제에서 UPDATE와 INSERT의 결속 및 같은 ref 원장 재시도 판정에 새 결함은 없습니다.  
[정합성] 만료 조회의 회원 결속·NULL 우선 정렬과 강의 summary의 hidden 제외는 유지됩니다.  
[경계] 50ms 타이머는 알림만 갱신하므로 확인 상태를 우회하지 않습니다. id의 문자는 안전하지만 여러 카드 간 고유성은 별도로 확보해야 합니다.  
[회귀] 제시된 데스크톱 행 구성에서는 `1fr`이 잔여 높이를 계속 흡수합니다. picks 없는 카드의 배치는 불변이며, 모바일 바닥 정렬은 기존 행 선언이 없어 확정할 수 없습니다.  
[회귀][nit] LOW `hyunhak-api/test/history_set_re.test.js:40` — “astra r1 사례”가 원래의 서로 다른 연속 구간 대신 네 접두어 모두 홀수 15편을 검사합니다.  
수정: 원래 반례인 01~15·02~16·03~17·04~18을 별도 회귀 사례로 추가합니다.  
[문안][nit] LOW `hyunhak-site/my.html:726` — 원장 실패로 응시 여부가 불명확해도 “아직 응시하지 않은 이용권”이라고 단정합니다.  
수정: 원장 실패 문구를 분기하거나 응시 여부 단정 없이 PDF 발급 효과를 설명합니다.  
집계: real 1건. 제공된 diff의 정적 검토이며, 보고된 테스트·렌더 결과는 재실행하지 않았습니다.  
근거: r1 다섯 건 중 네 건은 닫혔으나, 두 번째 PDF 버튼의 접근성 설명 연결 결함이 남았습니다.  
VERDICT: NO-GO
```
...
+});
+
+test("astra r1 원래 반례: 4접두어 01~15, 02~16, 03~17, 04~18 = 미보유 60세트 허용 0", () => {
+  const ids = PREFIXES.slice(0, 4).flatMap((p, i) => nos(1 + i, 1, 15).map((n) => p + n));
+  const { src, dropped, allowed } = check(ids);
+  assert.deepEqual(dropped, []);
+  assert.equal(allowed.size, 60);
+  for (const p of PREFIXES.slice(0, 4)) for (let n = 19; n <= 30; n++) assert.ok(!allowed.has(p + String(n)), `${p}${n} 허용됨: ${src}`);
+  assert.ok(!allowed.has("korea_2027_h16") && !allowed.has("korea_gorun_2027_s01"), src);
+});
+
...
+      var did=n>0, left=Math.max(0,SET_USES-n), armAttr=arm?' data-sdl-arm="'+arm+'"'+(noLedger?' data-sdl-unsure="1"':''):'';
...
+        // 채점된 응시는 행에서 바로 기록 열람으로 (critic P2, OU-7). 90일 지난 행은 영상과 첨삭이 없어 버튼을 내지 않는다
...
+  document.getElementById('stuRows').addEventListener('click', function(ev){ var b=ev.target.closest('.hgo'); if(!b) return; ev.preventDefault(); HH.studioGo(b); });
   ...
   (async function(){
@@ -702,11 +712,23 @@ document.addEventListener('DOMContentLoaded', async function(){
   })});
   // 세트 문제지·해설지 PDF (트랙 D, 2026-09-10): 행이 응시 원장 뒤에 그려지므로 위임 리스너. 서버가 권리·기간·한도를 판정하고
   // 구매자 이메일·주문번호·시리얼을 각인한 보안본을 준다. 멱등 idem 은 소장판과 같은 방식으로 sessionStorage 보존
-  var SDL_DOC={exam:'문제지',key:'해설지'};
+  var SDL_DOC={exam:'문제지',key:'해설지'}, sdlWarnSeq=0;   // 경고 id 일련번호: 카드가 달라도 같은 set_id 가 있을 수 있어 세트명만으로는 고유하지 않다
   document.getElementById('passList').addEventListener('click', async function(ev){
...
+      var row=b.closest('.r'), prev=row&&row.querySelector('.sr-warn'), wid=prev?prev.id:('sdlw_'+sid.replace(/[^a-z0-9_]/g,'')+'_'+(++sdlWarnSeq));
...
+      var msg=(b.dataset.sdlUnsure==='1'?'응시 여부를 지금 확인하지 못했습니다. 아직 응시하지 않은 이용권이면 문제지나 해설지 PDF 발급으로 제공이 개시되어 ':'아직 응시하지 않은 이용권입니다. 문제지나 해설지 PDF 를 발급받으면 제공이 개시되어 ')+scope+'의 청약철회가 제한됩니다(이용약관 제6조). 받으려면 버튼을 한 번 더 누르세요.';
+      if(row&&!prev){ var w=document.createElement('p'); w.className='sr-warn'; w.id=wid; w.textContent=msg; row.appendChild(w); }
...
+        // 입금 대기 주문은 서버가 PG 재조회로 계좌와 기한을 실어 준다 (R3-API-01: 성공 화면을 새로고침하면 계좌가 사라지던 결함의 복구 경로)
