...
# 구 URL 301 표 (2026-09-09, GA4 「없는 면」 51회 원인 = 08-26 철거 면접 아카이브가 답변엔진·색인에 잔존)
s, h, b = get("/interview/korea.html", follow=False); chk("/interview/korea.html → 301 programs/korea", s == 301 and h.get("Location", "").endswith("/programs/korea.html"), f"{s} {h.get('Location')}")
s, h, b = get("/interview/", follow=False); chk("/interview/ → 301 interview.html", s == 301 and h.get("Location", "").endswith("/interview.html"), f"{s} {h.get('Location')}")
s, h, b = get("/store.html", follow=False); chk("/store.html → 301 guidebook/", s == 301 and h.get("Location", "").endswith("/guidebook/index.html"), f"{s} {h.get('Location')}")
s, h, b = get("/favicon.ico"); chk("/favicon.ico 200 image/png", s == 200 and h.get("Content-Type", "").startswith("image/png"), f"{s} {h.get('Content-Type', '-')}")
