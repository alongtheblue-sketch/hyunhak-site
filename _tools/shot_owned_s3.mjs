// s3 (2026-09-14) 파생판: OU-5 사이트 분(입금 대기 주문 계좌·기한), OU-6(1) PDF 2단계 확인, OU-7(다편 목록·owned-note 접기·리포트 다시 보기·바닥글·앱 팔레트) 실렌더 증거.
// 원판 = shot_owned.mjs (s2). 스텁에 같은 단위 낱권 2편, 입금 대기 주문 1건을 더하고 프로브를 늘렸다.
...
const REPORT = path.join(SITE, '_docs', 'owned_ux_render_report_s3_20260914.json');
...
const OWNER = { member: MEMBER, trial_available: false,
  entitlements: [
    ...
    // s3: 같은 단위 낱권 2편째 (제목 없음 = studioTitle 폴백 검증) → studio.html 단위 카드가 owned-picks 목록을 그려야 한다 (R2-04)
    { id: 'ent_pass_kh5', kind: 'studio_passage', meta: JSON.stringify({ set_id: 'korea_2027_h05' }), uses_left: 5, expires_at: EXP },
...
const STUB = {
  '/api/config': { oauth: {} },
  // s3: 입금 대기 주문 1건 = 서버가 PG 재조회로 실어 주는 vaccount (R3-API-01). MY 주문 행에 은행·계좌·기한이 보여야 한다
  '/api/orders': { orders: [{ id: 'ord_stub_va', amount: 33000, status: 'waiting_deposit', created_at: '2026-09-14T02:00:00Z', paid_at: null, receipt_url: null,
    items: [{ title: '고려대 계열적합 인문 5번 지문 낱권', unit_price: 33000, qty: 1 }], vaccount: { bank: '우리은행', bankCode: '20', accountNumber: 'X1234567890', dueDate: '2026-09-21T23:59:59+09:00' } }] }, '/api/lectures': { lectures: [] }, '/api/lectures/public': { lectures: [] },
...
for (const state of ['owner', 'owner_done', 'trial', 'guest']) for (const w of [400, 1280]) {
  ...
  await ctx.route((u) => !/^(http:\/\/(localhost|127\.0\.0\.1)(:\d+)?\/|https:\/\/(api|studio)\.hyunhak\.com\/)/.test(u.href), (route) => route.fulfill({ status: 204, body: "" }));   // s3: 외부(폰트 CDN 등) 차단, 프록시 없는 셸에서 폰트 대기로 스크린샷 타임아웃
  const tokenCalls = [];
  ...
  for (const p of PAGES) {
    ...
    const m = await pg.evaluate(() => {
      ...
      return { ox: de.scrollWidth - de.clientWidth, height: de.scrollHeight, coarse: matchMedia('(pointer:coarse)').matches, own, cards,
        ...
        // s3 프로브
        noteDetails: (function(){ const d = document.querySelector('details.owned-note'); return d ? { open: d.open, summary: tx(d.querySelector('summary'), 80) } : null; })(),
        picks: q('.owned-picks li').map((li) => tx(li, 120)),
        histBtns: q('#stuRows .hgo').length,
        armBtns: q('#passList .sdl[data-sdl-arm]').length,
        orderRow: tx(document.querySelector('#orderRows li'), 300),
        ftLegal: q('footer .ft-legal a').map((a) => a.textContent.trim()), bizInfo: !!document.getElementById('business-info'),
        cardNames: q('#passList .subcard .nm').map((x) => tx(x, 80)) };
    });
    await pg.screenshot({ path: `${out}/${key}.png`, fullPage: true, timeout: 60000 });
    ...
    let sdl = null;
    if (p === 'my.html' && state === 'owner') {
      const pg3 = await ctx.newPage(); await pg3.goto('http://localhost:8811/' + p, { waitUntil: 'networkidle' }); await pg3.waitForTimeout(1200);
      const det = await pg3.$('#passList details.stsets'); if (det) await det.evaluate((d) => { d.open = true; });
      const btn = await pg3.$('#passList .sdl[data-sdl-arm]');
      if (btn) {
        const before = await btn.textContent(); await btn.click(); await pg3.waitForTimeout(300);
        sdl = { before: before.trim(), after: (await btn.textContent()).trim(), armed: await btn.getAttribute('data-sdl-armed'), scope: await btn.getAttribute('data-sdl-arm'), describedby: await btn.getAttribute('aria-describedby'), describedbyExists: await pg3.evaluate((b) => !!document.getElementById(b.getAttribute('aria-describedby')), btn), keyLink: await (async () => { const kb = await pg3.$('#passList .sdl[data-sdl-arm][data-doc="key"]'); if (!kb) return null; await kb.click(); await pg3.waitForTimeout(150); return pg3.evaluate((b) => ({ describedby: b.getAttribute('aria-describedby'), exists: !!document.getElementById(b.getAttribute('aria-describedby')), sameRow: b.closest('.r') === (document.getElementById(b.getAttribute('aria-describedby')) || {}).parentElement, warns: b.closest('.r').querySelectorAll('.sr-warn').length }), kb); })(), live: await pg3.evaluate(() => (document.getElementById('sdlLive') || {}).textContent || null),
          warn: await pg3.evaluate(() => { const w = document.querySelector('#passList .sr-warn'); return w ? w.textContent.slice(0, 160) : null; }) };
      } else sdl = { before: null, note: 'arm 버튼 없음' };
      await pg3.close();
    }
    report[key] = { ...m, errs, click, sdl };
...
for (const scope of ['', 'history']) for (const w of [400, 1280]) {
  const ctx = await b.newContext(w === 400 ? { ...devices['Pixel 7'], viewport: { width: 400, height: 860 } } : { viewport: { width: 1280, height: 900 } });
  await ctx.route((u) => !/^http:\/\/127\.0\.0\.1:8812\//.test(u.href), (route) => route.fulfill({ status: 204, body: "" }));   // s3: 앱도 외부 차단
  ...
  const m = await pg.evaluate(() => {
    ...
    return { ox: de.scrollWidth - de.clientWidth, height: de.scrollHeight, active: act ? act.id : null, setcards: q('.setcard').length, notices: q('.notice').length,
      ...
      // s3: 팔레트 = 사이트 토큰 (bg #F4EFE3 = rgb(244, 239, 227), ink #312E2E = rgb(49, 46, 46))
      bodyBg: getComputedStyle(document.body).backgroundColor, bodyInk: getComputedStyle(document.body).color };
  });
  await pg.screenshot({ path: `${out}/${key}.png`, fullPage: true, timeout: 60000 });
  await pg.close();
...
for (const [k, v] of Object.entries(report)) console.log(k, JSON.stringify({ ox: v.ox, errs: v.errs, coarse: v.coarse, own: v.own && v.own.slice(0, 160), trialGo: v.trialGo, libOwn: v.libOwn, tapSmall: v.tapSmall,
  ...
  firstUse: v.firstUse && v.firstUse.slice(0, 120), h1: v.h1, retry: v.retry, briefActive: v.briefActive, bodyBg: v.bodyBg, bodyInk: v.bodyInk,
  noteDetails: v.noteDetails, picks: v.picks, histBtns: v.histBtns, armBtns: v.armBtns, orderRow: v.orderRow, ftLegal: v.ftLegal, bizInfo: v.bizInfo, cardNames: v.cardNames, sdl: v.sdl,
