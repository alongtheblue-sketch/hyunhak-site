...
sql = ["-- 가이드북 38권 상품 시드 (_tools/build_products_sql.py 생성, 멱등)",
 ...
 "", "-- 성균관대 전권 이용권 (V2-8 폐기 확정 2026-08-27: inactive 고정, 재실행해도 되살아나지 않게)",
 f"INSERT INTO products(id,sku,type,title,subtitle,school,price,requires_shipping,stock,status,detail_url,sort,created_at,updated_at) VALUES ('prd_seed0005','pass-skku','pass_school','성균관대학교 전권 이용권','성균인재전형 제시문 전량, 스튜디오 무제한 응시','성균관대학교',330000,0,NULL,'inactive','/programs/skku.html',12,'{TS}','{TS}') ON CONFLICT(sku) DO UPDATE SET title=excluded.title, subtitle=excluded.subtitle, price=excluded.price, detail_url=excluded.detail_url, status=excluded.status, updated_at=excluded.updated_at;",
