...
const FIX = {
  ...
  orders: { orders: [{ created_at: '2026-08-26T12:00:00Z', paid_at: '2026-08-26T12:01:00Z', amount: 16500, status: 'paid',
...
await ctx.addInitScript(() => {
  try { localStorage.setItem('hh_cart_v1', JSON.stringify([{ sku: 'guide-korea', title: '고려대학교 2027 면접 가이드북', price: 16500, qty: 1, ship: false }])); } catch (e) {}
});
...
const PAGES = ['checkout.html', 'my.html', 'pay_done.html?paymentKey=stub_pk&orderId=HH-20260826-STUB&amount=16500'];
for (const p of PAGES) {
  ...
  const m = await pg.evaluate(() => {
    ...
    return { ox: de.scrollWidth - de.clientWidth, height: de.scrollHeight, url: location.pathname + location.search,
      ...
      marks: ['검수용', '고려대학교 2027 면접 가이드북', '16,500', '결제가 완료되었습니다', '결제 완료', '스텁 렌더'].filter((k) => t.includes(k)) };
  });
