...
INTRO = {
    ...
    "common": ["공통 풀이 4편은 연세대와 고려대 제시문 면접 판매 단위 가운데 고른기회를 뺀 열 단위에 걸리는 절차입니다. 이 시험이 지식 시험이 아니라 절차 시험이라는 전제, 결론 선언이 개수 계약이라는 구조 잡기, 7분을 편집으로 쓰는 말하기, 연습장의 빈칸이 절차라는 연습 시스템 총론입니다. 단위 전권에는 포함되어 있고, 응시 없이 절차만 들으려면 따로 삽니다."],
}
COMMON_TX = "공통 풀이 인강"
